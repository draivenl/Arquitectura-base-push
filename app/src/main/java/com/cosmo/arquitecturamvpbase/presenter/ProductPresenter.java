...
public class ProductPresenter extends BasePresenter<IProductView> {
    ...
    private void getProductList() {
        ...
        try {
            ...
            Log.i("Test Note XML name", note.getTo());
            Log.i("Test Note XML from", note.getFrom());
            Log.i("Test Note XML heading", note.getHeading());
            Log.i("Test Note XML body", note.getBody());


            BreakfastMenuRepository breakfastMenuRepository = new BreakfastMenuRepository();
            BreakfastMenu breakfastMenu = breakfastMenuRepository.getBreakfastMenu();
            ArrayList<Food> foods = breakfastMenu.getFoods();
            for (Food food : foods) {
                Log.i("Foooods", "Food Name:::" + food.getName());
            }
